...
class CablepickingControllerClient(controllerclientbase.ControllerClientBase):
    """mujin controller client for cable picking task
    """
    tasktype = 'cablepicking'
    def __init__(self, controllerurl, controllerusername, controllerpassword, robotControllerIp, robotControllerPort, cablepickingzmqport, cablepickingheartbeatport, cablepickingheartbeattimeout, scenepk, robotname, robotspeed, regionname, targetname, toolname, envclearance, usewebapi=False, initializezmq=False):
        """logs into the mujin controller, initializes cablepicking task, and sets up parameters
        ...
        :param cablepickingzmqport: port of the cablepicking task's zmq server, e.g. 7110
        :param cablepickingheartbeatport: port of the cablepicking task's zmq server's heartbeat publisher, e.g. 7111
        :param cablepickingheartbeattimeout: seconds until reinitializing cablepicking task's zmq server if no hearbeat is received, e.g. 7
        :param scenepk: pk of the cable picking task scene, e.g. irex2013.mujin.dae
        ...
        """
        super(CablepickingControllerClient, self).__init__(controllerurl, controllerusername, controllerpassword, cablepickingzmqport, cablepickingheartbeatport, cablepickingheartbeattimeout, self.tasktype, scenepk, initializezmq)
        ...
        # whether to use webapi for cable picking task commands
    ...
    def InitializePartsWithPhysics(self, **kwargs):
        """Start a physics simulation where the parts drop down into the cable. The method returns as soon as the physics is initialized, user has to wait for the "duration" or call StopPhysicsThread command.
    ...
    def GetTrajectoryLog(self,**kwargs):
        """Gets the recent trajectories executed on the cablepicking server. The internal server keeps trajectories around for 10 minutes before clearing them.
    ...
    def GetPickAndPlaceLog(self,**kwargs):
        """Gets the recent pick-and-place log executed on the cablepicking server. The internal server keeps the log around until the next Pick-and-place command is executed.
