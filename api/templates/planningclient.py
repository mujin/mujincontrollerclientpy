...
def ParseControllerInfo(controllerUrl, username, password):
    """Return controller URL, username, password, port and IP/hostname. This is done because the URL can contain the information.
    """
    # Parse controllerUrl
...
class PlanningControllerClient(controllerclientbase.ControllerClient):
    ...
    _sceneparams = None
    scenepk = None  # The scenepk this client is configured for
    ...
    _taskheartbeatport = None  # Port of the task's zmq server's heartbeat publisher, e.g. 7111
    _taskheartbeattimeout = None  # Seconds until reinitializing task's zmq server if no heartbeat is received, e.g. 7

    def __init__(self, taskzmqport=7110, taskheartbeatport=7111, taskheartbeattimeout=7.0, tasktype='', scenepk='', ctx=None, slaverequestid=None, controllerurl='http://127.0.0.1', controllerusername='', controllerpassword='', author=None):
        """Logs into the Mujin controller and initializes the connection to the planning server (ZMQ).
        ...
        self._sceneparams = {}
        self._isok = True
        super(PlanningControllerClient, self).__init__(controllerurl=controllerurl, controllerusername=controllerusername, controllerpassword=controllerpassword, author=author)
    ...
    def ExecuteCommand(self, taskparameters, slaverequestid=None, timeout=None, fireandforget=None, respawnopts=None, checkpreempt=True):
        ...
        Args:
            ...
            usewebapi (bool, optional): If True, sends the command via the web API instead of the ZMQ API.. Defaults to None.
    ...
    def SetLogLevel(self, componentLevels, fireandforget=None, timeout=5):
        """Set planning log level. Webstack log level is not affected (configure it separately).
    ...
    #
    # Viewer Parameters Related (to be deprecated with UI v4)
    #
