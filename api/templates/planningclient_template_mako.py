...
from . import APIServerErrorGetMonotonicTime
...
class ${clientClassName}(controllerclientbase.ControllerClient):
    ...
    def __init__(self, ${constructorArgs}):
        ...
        """
        super(${clientClassName}, self).__init__(**kwargs)
        ${constructorContent}
    ...
    def SendConfig(self, command, slaverequestid=None, timeout=None, fireandforget=None):
    ...
    def SetViewerFromParameters(self, viewerparameters, timeout=10, fireandforget=True, **kwargs):
        ...
        confcommand = dict()
        confcommand['command'] = 'configure'
        confcommand['viewerparameters'] = viewerparameters
        slaverequestid = self._slaverequestid
        confcommand['slaverequestid'] = slaverequestid
        return self.SendZMQCommand(confcommand, timeout=timeout, fireandforget=fireandforget, checkpreempt=True)
    ...
    def MoveCameraZoomOut(self, zoommult=0.9, zoomdelta=20, timeout=10, fireandforget=True, ispan=True, **kwargs):
        ...
        confcommand = dict()
        confcommand['command'] = 'configure'
        confcommand['viewercommand'] = viewercommand
        slaverequestid = self._slaverequestid
        confcommand['slaverequestid'] = slaverequestid
        return self.SendZMQCommand(confcommand, timeout=timeout, fireandforget=fireandforget, checkpreempt=True)
    ...
    def StartIPython(self, timeout=1, fireandforget=True, **kwargs):
        ...
        configuration['command'] = 'configure'
        configuration['slaverequestid'] = self._slaverequestid
        response = self._configsocket.SendCommand(configuration, timeout=timeout, fireandforget=fireandforget, checkpreempt=True)
        if fireandforget:
            # For fire and forget commands, no response will be available
            return None
